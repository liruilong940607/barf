...
class Dataset(base.Dataset):
    def __init__(self, opt, split="train", subset=None):
        ...
        self.focal = (
            0.5 * self.raw_W / np.tan(0.5 * self.meta["camera_angle_x"])
        )
        if subset:
        ...
        if opt.data.preload:
            ...
            self.cameras = self.preload_threading(
                opt, self.get_camera, data_str="cameras"
            )

    def prefetch_all_data(self, opt):
        assert not opt.data.augment
        # pre-iterate through all samples and group together
        self.all = torch.utils.data._utils.collate.default_collate(
            [s for s in self]
        )

    def get_all_camera_poses(self, opt):
        pose_raw_all = [
            torch.tensor(f["transform_matrix"], dtype=torch.float32)
            for f in self.list
        ]
        pose_canon_all = torch.stack(
            [self.parse_raw_camera(opt, p) for p in pose_raw_all], dim=0
        )
    ...
    def __getitem__(self, idx):
        ...
        image = (
            self.images[idx] if opt.data.preload else self.get_image(opt, idx)
        )
        image = self.preprocess_image(opt, image, aug=aug)
        intr, pose = (
            self.cameras[idx]
            if opt.data.preload
            else self.get_camera(opt, idx)
        )
    ...
    def get_image(self, opt, idx):
        image_fname = "{}/{}.png".format(
            self.path, self.list[idx]["file_path"]
        )
        image = PIL.Image.fromarray(
            imageio.imread(image_fname)
    ...
    def get_camera(self, opt, idx):
        intr = torch.tensor(
            [
                [self.focal, 0, self.raw_W / 2],
                [0, self.focal, self.raw_H / 2],
                [0, 0, 1],
            ]
        ).float()
        pose_raw = torch.tensor(
            self.list[idx]["transform_matrix"], dtype=torch.float32
        )
