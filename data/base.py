...
class Dataset(torch.utils.data.Dataset):
    ...
    def preload_threading(self, opt, load_func, data_str="images"):
        ...
        idx_tqdm = tqdm.tqdm(
            range(len(self)),
            desc="preloading {}".format(data_str),
            leave=False,
        )
        ...
        for ti in range(opt.data.num_workers):
            t = threading.Thread(
                ...
                args=(data_list, load_func, q, lock, idx_tqdm),
                daemon=True,
            )
            t.start()
        q.join()
    ...
    def generate_augmentation(self, opt):
        brightness = opt.data.augment.brightness or 0.0
        contrast = opt.data.augment.contrast or 0.0
        saturation = opt.data.augment.saturation or 0.0
        hue = opt.data.augment.hue or 0.0
        ...
        aug = edict(
            ...
            rot_angle=(np.random.rand() * 2 - 1) * opt.data.augment.rotate
            if opt.data.augment.rotate
            else 0,
        )
        return aug
    ...
    def preprocess_image(self, opt, image, aug=None):
        ...
        if opt.data.center_crop is not None:
            self.crop_H = int(self.raw_H * opt.data.center_crop)
            self.crop_W = int(self.raw_W * opt.data.center_crop)
            image = torchvision_F.center_crop(
                image, (self.crop_H, self.crop_W)
            )
        else:
            self.crop_H, self.crop_W = self.raw_H, self.raw_W
        # resize
