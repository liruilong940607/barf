...
class Model:
    ...
    def load_dataset(self, opt, eval_split="val"):
        ...
        self.train_data = data.Dataset(
            opt, split="train", subset=opt.data.train_sub
        )
        self.train_loader = self.train_data.setup_loader(opt, shuffle=True)
        log.info("loading test data...")
        if opt.data.val_on_test:
            eval_split = "test"
        self.test_data = data.Dataset(
            opt, split=eval_split, subset=opt.data.val_sub
        )
    ...
    def setup_optimizer(self, opt):
        ...
        self.optim = optimizer(
            [dict(params=self.graph.parameters(), lr=opt.optim.lr)]
        )
        # set up scheduler
        if opt.optim.sched:
    ...
    def restore_checkpoint(self, opt):
        ...
        if opt.resume:
            ...
            epoch_start, iter_start = util.restore_checkpoint(
    ...
    def setup_visualizer(self, opt):
        ...
        if opt.tb:
            self.tb = torch.utils.tensorboard.SummaryWriter(
                log_dir=opt.output_path, flush_secs=10
            )
        if opt.visdom:
            # check if visdom server is runninng
            is_open = util.check_socket_open(
                opt.visdom.server, opt.visdom.port
            )
            retry = None
            while not is_open:
                retry = input(
            ...
            self.vis = visdom.Visdom(
                server=opt.visdom.server, port=opt.visdom.port, env=opt.group
            )
    ...
    def train_epoch(self, opt):
        ...
        loader = tqdm.tqdm(
            self.train_loader,
            desc="training epoch {}".format(self.ep + 1),
            leave=False,
        )
        for batch in loader:
            # train iteration
        ...
        if opt.optim.sched:
            self.sched.step()
        if (self.ep + 1) % opt.freq.val == 0:
            self.validate(opt, ep=self.ep + 1)
    ...
    def train_iteration(self, opt, var, loader):
        ...
        if self.graph.nerf.use_occ_grid:
            self.graph.nerf.occ_grid.every_n_step(
                step=self.it,
        ...
        #  print(self.graph.nerf.occ_grid.binary.float().mean())
    ...
    def save_checkpoint(self, opt, ep=0, it=0, latest=False):
        ...
        if not latest:
            log.info(
                "checkpoint saved: ({0}) {1}, epoch {2} (iteration {3})".format(
