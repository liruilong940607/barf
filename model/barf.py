...
class Model(nerf.Model):
    ...
    def build_networks(self, opt):
        ...
        if opt.camera.noise:
            ...
            se3_noise = (
                torch.randn(len(self.train_data), 6, device=opt.device)
                * opt.camera.noise
            )
            self.graph.pose_noise = camera.lie.se3_to_SE3(se3_noise)
        self.graph.se3_refine = torch.nn.Embedding(len(self.train_data), 6).to(
            opt.device
        )
    ...
    def setup_optimizer(self, opt):
        ...
        self.optim_pose = optimizer(
            [
                dict(
                    params=self.graph.se3_refine.parameters(),
        ...
        if opt.optim.sched_pose:
            scheduler = getattr(
                torch.optim.lr_scheduler, opt.optim.sched_pose.type
            )
            if opt.optim.lr_pose_end:
                assert opt.optim.sched_pose.type == "ExponentialLR"
                opt.optim.sched_pose.gamma = (
                    opt.optim.lr_pose_end / opt.optim.lr_pose
                ) ** (1.0 / opt.max_iter)
            kwargs = {
                k: v for k, v in opt.optim.sched_pose.items() if k != "type"
            }
    ...
    def train_iteration(self, opt, var, loader):
        ...
        if opt.optim.warmup_pose:
            ...
            self.optim_pose.param_groups[0][
                "lr_orig"
            ] = self.optim_pose.param_groups[0][
                "lr"
            ]  # cache the original learning rate
            self.optim_pose.param_groups[0]["lr"] *= min(
                1, self.it / opt.optim.warmup_pose
            )
    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        if split == "train" and opt.data.dataset in ["blender", "llff"]:
            ...
            self.tb.add_scalar(
                "{0}/error_R".format(split), error.R.mean(), step
            )
            self.tb.add_scalar(
                "{0}/error_t".format(split), error.t.mean(), step
            )
    ...
    def visualize(self, opt, var, step=0, split="train"):
        ...
        if opt.visdom:
            if split == "val":
                pose, pose_GT = self.get_all_training_poses(opt)
                util_vis.vis_cameras(
                    opt, self.vis, step=step, poses=[pose, pose_GT]
                )
    ...
    def evaluate_full(self, opt):
        ...
        pose, pose_GT = self.get_all_training_poses(opt)
        pose_aligned, self.graph.sim3 = self.prealign_cameras(
            opt, pose, pose_GT
        )
        error = self.evaluate_camera_alignment(opt, pose_aligned, pose_GT)
        print("--------------------------")
    ...
    def evaluate_test_time_photometric_optim(self, opt, var):
        ...
        var.se3_refine_test = torch.nn.Parameter(
            torch.zeros(1, 6, device=opt.device)
        )
        optimizer = getattr(torch.optim, opt.optim.algo)
        optim_pose = optimizer(
            [dict(params=[var.se3_refine_test], lr=opt.optim.lr_pose)]
        )
        iterator = tqdm.trange(
            opt.optim.test_iter,
            desc="test-time optim.",
            leave=False,
            position=1,
        )
        for it in iterator:
    ...
    def generate_videos_pose(self, opt):
        self.graph.eval()
        fig = plt.figure(
            figsize=(10, 10) if opt.data.dataset == "blender" else (16, 8)
        )
        ...
        for ep in range(0, opt.max_iter + 1, opt.freq.ckpt):
            ...
            if opt.data.dataset in ["blender", "llff"]:
                ...
                util_vis.plot_save_poses(
                    opt, fig, pose, pose_ref=None, path=cam_path, ep=ep
                )
            ep_list.append(ep)
        plt.close()
        # write videos
        ...
        os.system(
            "ffmpeg -y -r 30 -f concat -i {0} -pix_fmt yuv420p {1} >/dev/null 2>&1".format(
                list_fname, cam_vid_fname
            )
        )
