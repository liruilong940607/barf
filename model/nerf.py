    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        if split == "train":
            ...
            if opt.nerf.fine_sampling:
                ...
                self.tb.add_scalar(
                    "{0}/{1}".format(split, "lr_fine"), lr, step
                )
        # compute PSNR
        ...
        if opt.nerf.fine_sampling:
            ...
            self.tb.add_scalar(
                "{0}/{1}".format(split, "PSNR_fine"), psnr, step
            )
...
class Graph(base.Graph):
    ...
    def render_by_slices(self, opt, pose, intr=None, mode=None):
        ...
        for c in range(0, opt.H * opt.W, opt.nerf.rand_rays):
            ray_idx = torch.arange(
                c,
                min(c + opt.nerf.rand_rays, opt.H * opt.W),
                device=opt.device,
            )
            ret = self.render(
    ...
    def sample_depth(self, opt, batch_size, num_rays=None):
        ...
        rand_samples = (
            torch.rand(
                batch_size,
                num_rays,
                opt.nerf.sample_intvs,
                1,
                device=opt.device,
            )
            if opt.nerf.sample_stratified
            else 0.5
        )
        rand_samples += torch.arange(opt.nerf.sample_intvs, device=opt.device)[
            None, None, :, None
        ].float()  # [B,HW,N,1]
        depth_samples = (
            rand_samples / opt.nerf.sample_intvs * (depth_max - depth_min)
            + depth_min
        )  # [B,HW,N,1]
        depth_samples = dict(
    ...
    def sample_depth_from_pdf(self, opt, pdf):
        ...
        grid = torch.linspace(
            0, 1, opt.nerf.sample_intvs_fine + 1, device=opt.device
        )  # [Nf+1]
        unif = 0.5 * (grid[:-1] + grid[1:]).repeat(
            *cdf.shape[:-1], 1
        )  # [B,HW,Nf]
        ...
        depth_bin = torch.linspace(
            depth_min, depth_max, opt.nerf.sample_intvs + 1, device=opt.device
        )  # [N+1]
        depth_bin = depth_bin.repeat(*cdf.shape[:-1], 1)  # [B,HW,N+1]
        depth_low = depth_bin.gather(
            dim=2, index=(idx - 1).clamp(min=0)
        )  # [B,HW,Nf]
        depth_high = depth_bin.gather(
            dim=2, index=idx.clamp(max=opt.nerf.sample_intvs)
        )  # [B,HW,Nf]
...
class NeRF(torch.nn.Module):
    ...
    def define_network(self, opt):
        ...
        if opt.nerf.view_dep:
            input_view_dim = (
                3 + 6 * opt.arch.posenc.L_view if opt.arch.posenc else 3
            )
        # point-wise feature
        ...
        for li, (k_in, k_out) in enumerate(L):
            ...
            if opt.arch.tf_init:
                self.tensorflow_init_weights(
                    opt, linear, out="first" if li == len(L) - 1 else None
                )
        ...
        for li, (k_in, k_out) in enumerate(L):
            ...
            if opt.arch.tf_init:
                self.tensorflow_init_weights(
                    opt, linear, out="all" if li == len(L) - 1 else None
                )
        ...
        for li, layer in enumerate(self.mlp_feat):
            ...
            if li == len(self.mlp_feat) - 1:
                ...
                if opt.nerf.density_noise_reg and mode == "train":
                    density += (
                        torch.randn_like(density) * opt.nerf.density_noise_reg
                    )
                density_activ = getattr(
                    torch_F, opt.arch.density_activ
        ...
        if opt.nerf.view_dep:
            ...
            if opt.arch.posenc:
                ray_enc = self.positional_encoding(
                    opt, ray_unit, L=opt.arch.posenc.L_view
                )
                ray_enc = torch.cat(
                    [ray_unit, ray_enc], dim=-1
                )  # [B,...,6L+3]
            else:
                ray_enc = ray_unit
    ...
    def forward_samples(self, opt, center, ray, depth_samples, mode=None):
        points_3D_samples = camera.get_3D_points_from_depth(
            opt, center, ray, depth_samples, multi_samples=True
        )  # [B,HW,N,3]
        if opt.nerf.view_dep:
        ...
        rgb_samples, density_samples = self.forward(
            opt, points_3D_samples, ray_unit=ray_unit_samples, mode=mode
        )  # [B,HW,N],[B,HW,N,3]
    ...
    def composite(self, opt, ray, rgb_samples, density_samples, depth_samples):
        ...
        depth_intv_samples = (
            depth_samples[..., 1:, 0] - depth_samples[..., :-1, 0]
        )  # [B,HW,N-1]
