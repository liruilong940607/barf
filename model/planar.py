...
class Model(base.Model):
    ...
    def build_networks(self, opt):
        ...
        self.graph.warp_param = torch.nn.Embedding(
            opt.batch_size, opt.warp.dof
        ).to(opt.device)
    ...
    def setup_optimizer(self, opt):
        ...
        optim_list = [
            ...
            dict(
                params=self.graph.warp_param.parameters(), lr=opt.optim.lr_warp
            ),
        ]
    ...
    def train(self, opt):
        ...
        # after training
        os.system(
            "ffmpeg -y -framerate 30 -i {}/%d.png -pix_fmt yuv420p {}".format(
                self.vis_path, self.video_fname
            )
        )
    ...
    def generate_warp_perturbation(self, opt):
        ...
        warp_pert_all = torch.zeros(
            opt.batch_size, opt.warp.dof, device=opt.device
        )
        trans_pert = [(0, 0)] + [
            (x, y)
            for x in (-opt.warp.noise_t, opt.warp.noise_t)
            for y in (-opt.warp.noise_t, opt.warp.noise_t)
        ]

        def create_random_perturbation():
            warp_pert = (
                torch.randn(opt.warp.dof, device=opt.device) * opt.warp.noise_h
            )
        ...
        xy_grid_warped = xy_grid_warped.view(
            [opt.batch_size, opt.H_crop, opt.W_crop, 2]
        )
        xy_grid_warped = torch.stack(
            [
                xy_grid_warped[..., 0] * max(opt.H, opt.W) / opt.W,
                xy_grid_warped[..., 1] * max(opt.H, opt.W) / opt.H,
            ],
            dim=-1,
        )
        image_raw_batch = self.image_raw.repeat(opt.batch_size, 1, 1, 1)
        image_pert_all = torch_F.grid_sample(
            image_raw_batch, xy_grid_warped, align_corners=False
        )
    ...
    def visualize_patches(self, opt, warp_param):
        ...
        corners_all[..., 0] = (
            corners_all[..., 0] / opt.W * max(opt.H, opt.W) + 1
        ) / 2 * opt.W - 0.5
        corners_all[..., 1] = (
            corners_all[..., 1] / opt.H * max(opt.H, opt.W) + 1
        ) / 2 * opt.H - 0.5
        for i, corners in enumerate(corners_all):
            P = [tuple(float(n) for n in corners[j]) for j in range(4)]
            draw.line(
                [P[0], P[1], P[2], P[3], P[0]],
                fill=tuple(self.box_colors[i]),
                width=3,
            )
    ...
    def log_scalars(self, opt, var, loss, metric=None, step=0, split="train"):
        ...
        # warp error
        warp_error = (
            (self.graph.warp_param.weight - self.warp_pert).norm(dim=-1).mean()
        )
        self.tb.add_scalar(
            "{0}/{1}".format(split, "warp error"), warp_error, step
        )
    ...
    def visualize(self, opt, var, step=0, split="train"):
        ...
        frame_cat = (
            (torch.cat([frame, frame2], dim=1) * 255)
            .byte()
            .permute(1, 2, 0)
            .numpy()
        )
        imageio.imsave(
            "{}/{}.png".format(self.vis_path, self.vis_it), frame_cat
        )
        self.vis_it += 1
        ...
        if opt.tb:
            ...
            util_vis.tb_image(
                ...
                split,
                "image_pert",
                util_vis.color_border(var.image_pert, colors),
            )
            util_vis.tb_image(
                opt,
                self.tb,
                ...
                split,
                "rgb_warped",
                util_vis.color_border(var.rgb_warped_map, colors),
            )
            util_vis.tb_image(
    ...
    def compute_loss(self, opt, var, mode=None):
        ...
        if opt.loss_weight.render is not None:
            image_pert = var.image_pert.view(
                opt.batch_size, 3, opt.H_crop * opt.W_crop
            ).permute(0, 2, 1)
...
class NeuralImageFunction(torch.nn.Module):
    ...
    def forward(self, opt, coord_2D):  # [B,...,3]
        if opt.arch.posenc:
            points_enc = self.positional_encoding(
                opt, coord_2D, L=opt.arch.posenc.L_2D
            )
            points_enc = torch.cat(
                [coord_2D, points_enc], dim=-1
            )  # [B,...,6L+3]
        else:
            points_enc = coord_2D
        feat = points_enc
    ...
    def positional_encoding(self, opt, input, L):  # [B,...,N]
        shape = input.shape
        freq = (
            2 ** torch.arange(L, dtype=torch.float32, device=opt.device)
            * np.pi
        )  # [L]
