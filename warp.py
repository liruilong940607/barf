...
def get_normalized_pixel_grid(opt):
    y_range = (
        (torch.arange(opt.H, dtype=torch.float32, device=opt.device) + 0.5)
        / opt.H
        * 2
        - 1
    ) * (opt.H / max(opt.H, opt.W))
    x_range = (
        (torch.arange(opt.W, dtype=torch.float32, device=opt.device) + 0.5)
        / opt.W
        * 2
        - 1
    ) * (opt.W / max(opt.H, opt.W))
...
def get_normalized_pixel_grid_crop(opt):
    ...
    y_range = (
        (torch.arange(*(y_crop), dtype=torch.float32, device=opt.device) + 0.5)
        / opt.H
        * 2
        - 1
    ) * (opt.H / max(opt.H, opt.W))
    x_range = (
        (torch.arange(*(x_crop), dtype=torch.float32, device=opt.device) + 0.5)
        / opt.W
        * 2
        - 1
    ) * (opt.W / max(opt.H, opt.W))
...
def warp_corners(opt, warp_param):
    ...
    x_crop = (opt.W // 2 - opt.W_crop // 2, opt.W // 2 + opt.W_crop // 2)
    Y = [
        ((y + 0.5) / opt.H * 2 - 1) * (opt.H / max(opt.H, opt.W))
        for y in y_crop
    ]
    X = [
        ((x + 0.5) / opt.W * 2 - 1) * (opt.W / max(opt.H, opt.W))
        for x in x_crop
    ]
    corners = [(X[0], Y[0]), (X[0], Y[1]), (X[1], Y[1]), (X[1], Y[0])]
    corners = torch.tensor(
        corners, dtype=torch.float32, device=opt.device
...
class Lie:
    def so2_to_SO2(self, theta):  # [...,1]
        thetax = torch.stack(
            [
                torch.cat([theta.cos(), -theta.sin()], dim=-1),
                torch.cat([theta.sin(), theta.cos()], dim=-1),
            ],
            dim=-2,
        )
        R = thetax
    ...
    def se2_to_SE2(self, delta):  # [...,3]
        ...
        V = torch.stack(
            [torch.cat([A, -B], dim=-1), torch.cat([B, A], dim=-1)], dim=-2
        )
    ...
    def se2_jacobian(self, X, delta):  # [...,N,2],[...,3]
        ...
        R = self.so2_to_SO2(theta)
        dV_dtheta = torch.stack(
            [torch.cat([C, -D], dim=-1), torch.cat([D, C], dim=-1)], dim=-2
        )  # [...,2,2]
