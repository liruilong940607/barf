...
def tb_image(
    ...
    images = preprocess_vis_image(
        opt, images, from_range=from_range, cmap=cmap
    )
...
def dump_images(
    opt, idx, name, images, masks=None, from_range=(0, 1), cmap="gray"
):
    images = preprocess_vis_image(
        opt, images, masks=masks, from_range=from_range, cmap=cmap
    )  # [B,3,H,W]
...
def get_heatmap(opt, gray, cmap):  # [N,H,W]
    ...
    color = (
        torch.from_numpy(color[..., :3]).permute(0, 3, 1, 2).float()
    )  # [N,3,H,W]
    return color
...
def color_border(images, colors, width=3):
    ...
    for i, image in enumerate(images):
        image_pad = torch.ones(
            3, image.shape[1] + width * 2, image.shape[2] + width * 2
        ) * (colors[i, :, None, None] / 255.0)
...
@torch.no_grad()
def vis_cameras(
    opt, vis, step, poses=[], colors=["blue", "magenta"], plot_dist=True
):
    ...
    for pose, color in zip(poses, colors):
        ...
        vertices, faces, wireframe = get_camera_mesh(
            pose, depth=opt.visdom.cam_depth
        )
        center = vertices[:, -1]
    ...
    vis._send(
        dict(
            ...
            opts=dict(
                title="{} poses ({})".format(win_name, step),
            ),
        )
    )
...
def merge_meshes(vertices, faces):
    ...
    faces_merged = torch.cat(
        [faces + i * vertex_N for i in range(mesh_N)], dim=0
    )
...
def plot_save_poses(opt, fig, pose, pose_ref=None, path=None, ep=None):
    ...
    setup_3D_plot(
        ax1, elev=-90, azim=-90, lim=edict(x=(-1, 1), y=(-1, 1), z=(-1, 1))
    )
    setup_3D_plot(
        ax2, elev=0, azim=-90, lim=edict(x=(-1, 1), y=(-1, 1), z=(-1, 1))
    )
    ax1.set_title("forward-facing view", pad=0)
    ax2.set_title("top-down view", pad=0)
    plt.subplots_adjust(
        left=0, right=1, bottom=0, top=0.95, wspace=0, hspace=0
    )
...
def plot_save_poses_blender(opt, fig, pose, pose_ref=None, path=None, ep=None):
    ...
    setup_3D_plot(
        ax, elev=45, azim=35, lim=edict(x=(-3, 3), y=(-3, 3), z=(-3, 2.4))
    )
    plt.subplots_adjust(
        left=0, right=1, bottom=0, top=0.95, wspace=0, hspace=0
    )
    ...
    if ep == 0:
        ...
        plt.savefig(png_fname, dpi=75)
    ax.add_collection3d(
        Poly3DCollection([v[:4] for v in cam], alpha=0.2, facecolor=pred_color)
    )
    for i in range(N):
        ax.plot(
            cam[i, :, 0],
        ...
        ax.scatter(
            cam[i, 5, 0], cam[i, 5, 1], cam[i, 5, 2], color=pred_color, s=20
        )
    for i in range(N):
        ax.plot(
            [cam[i, 5, 0], cam_ref[i, 5, 0]],
            [cam[i, 5, 1], cam_ref[i, 5, 1]],
            [cam[i, 5, 2], cam_ref[i, 5, 2]],
            color=(1, 0, 0),
            linewidth=3,
        )
    png_fname = "{}/{}.png".format(path, ep)
