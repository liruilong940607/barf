...
class Log:
    ...
    def loss_train(self, opt, ep, lr, loss, timer):
        ...
        message += ", time:{}".format(
            blue(
                "{0}-{1:02d}:{2:02d}:{3:02d}".format(*get_time(timer.elapsed)),
                bold=True,
            )
        )
        message += " (ETA:{})".format(
            blue(
                "{0}-{1:02d}:{2:02d}:{3:02d}".format(*get_time(timer.arrival))
            )
        )
        print(message)
...
def update_timer(opt, timer, ep, it_per_ep):
    ...
    timer.it_mean = (
        timer.it_mean * momentum + timer.it * (1 - momentum)
        if timer.it_mean is not None
        else timer.it
    )
...
def move_to_device(X, device):
    ...
    elif isinstance(X, tuple) and hasattr(
        X, "_fields"
...
def save_checkpoint(opt, model, ep, it, latest=False, children=None):
    ...
    if children is not None:
        graph_state_dict = {
            k: v
            for k, v in model.graph.state_dict().items()
